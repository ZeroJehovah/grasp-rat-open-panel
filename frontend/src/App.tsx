...
function killPosition(kill: Kill, map: MapMetadata): { text: string; color: string } {
...
function MainPanel({ realtime, history, meta, range, loading, status }: { realtime: RealtimeResponse | null; history: HistoryResponse | null; meta: MetaResponse; range: { from: string; to: string; preset?: RangePreset }; loading: boolean; status: ReactNode }) {
  ...
  return <main className="main-grid">{status}<aside className="left-rail"><RangeControls meta={meta} /><ChatPanel messages={messages} /></aside><section className="right-field"><nav className="tabs" aria-label="面板视图"><button className={tab === 'map' ? 'tab active' : 'tab'} onClick={() => setTab('map')}><MapIcon size={15} />实时地图</button><button className={tab === 'players' ? 'tab active' : 'tab'} onClick={() => setTab('players')}><Users size={15} />玩家列表</button><button className={tab === 'kills' ? 'tab active' : 'tab'} onClick={() => setTab('kills')}><Swords size={15} />击杀明细</button><span className="tab-status">{loading ? <><RefreshCw size={13} className="spin" />同步历史</> : <><span className="status-dot" />{realtime ? `v${realtime.latest?.server_tick ?? '--'}` : '等待快照'}</>}</span></nav>{tab === 'map' && <MapView players={realtime ? realtimePlayers : []} map={realtime?.map || meta.map} />}{tab === 'players' && <section className="players-panel panel-block"><div className="section-heading"><div><p className="eyebrow">RANKED CURRENT STATE</p><h2>玩家列表</h2></div><span className="result-count">{players.length} 位玩家</span></div><PlayersTable players={players} map={meta.map} rangeLabel={rangeLabel} /></section>}{tab === 'kills' && <KillTable kills={kills} map={meta.map} />}</section></main>;
}
...
export default function App() {
  ...
  const status = <div className="global-status">{metaError ? <><CircleAlert size={14} />元数据加载失败：{metaError}</> : data.versionError ? <><CircleAlert size={14} />实时版本检查失败，显示旧缓存{realtimeAge}</> : data.historyError ? <><CircleAlert size={14} />历史数据暂不可用，实时视图仍可用{realtimeAge}</> : <><span className="status-dot" />服务运行中 · {meta?.timezone || 'Asia/Shanghai'}{realtimeAge}</>}</div>;
  return <div className="app-shell"><Banner theme={theme} onTheme={() => setTheme(value => value === 'light' ? 'dark' : 'light')} />{meta ? <MainPanel realtime={data.realtime} history={data.history} meta={meta} range={range} loading={data.loadingHistory} status={status} /> : <main className="loading-screen"><RefreshCw className="spin" size={20} /><span>正在连接观测数据…</span></main>}<Footer /></div>;
}
