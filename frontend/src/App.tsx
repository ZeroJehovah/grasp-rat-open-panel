import { Fragment, useEffect, useMemo, useRef, useState, type ReactNode, type RefObject, type UIEvent, type WheelEvent } from 'react';
...
const DROP_THRESHOLD_MAX_FLOOR = 100;

function dropThresholdMax(value: number, observedMax: number): number {
  return Math.max(DROP_THRESHOLD_MAX_FLOOR, value + DROP_THRESHOLD_MAX_FLOOR, Math.ceil(observedMax) + 10);
}

function DropThresholdSlider({ value, observedMax, ariaLabel, onChange }: { value: number; observedMax: number; ariaLabel: string; onChange: (value: number) => void }) {
  const max = dropThresholdMax(value, observedMax);
  const updateValue = (next: number) => {
    if (Number.isInteger(next) && next >= DROP_THRESHOLD_MIN && next <= max) onChange(next);
  };
  const handleWheel = (event: WheelEvent<HTMLElement>) => {
    if (event.deltaY === 0) return;
    event.preventDefault();
    updateValue(value + (event.deltaY < 0 ? 1 : -1));
  };
  return <label className="drop-threshold-control" title="拖动滑块或悬停后滚动鼠标滚轮调整 Drop 阈值"><span>Drop ≥</span><input className="drop-threshold-slider" type="range" min={DROP_THRESHOLD_MIN} max={max} step="1" value={value} aria-label={ariaLabel} onChange={event => updateValue(Number(event.currentTarget.value))} onWheel={handleWheel} /><output className="drop-threshold-value">{displayNumber(value)}</output></label>;
}
...
function MapView({ players, map }: { players: MapPlayer[]; map: MapMetadata }) {
  ...
  const observedMaxDrop = players.reduce((max, player) => player.drop === null ? max : Math.max(max, player.drop), 0);
...
function KillTable({ kills, map }: { kills: Kill[]; map: MapMetadata }) {
  ...
  const observedMaxDrop = kills.reduce((max, kill) => kill.drop?.amount === null || kill.drop?.amount === undefined ? max : Math.max(max, kill.drop.amount), 0);
