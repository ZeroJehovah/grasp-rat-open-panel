...
const { RAW_SNAPSHOT_LIMIT, listRawSnapshots } = require('./raw-retention');
...
function rawRetentionBacklog(directory) {
  let count = 0;
  let bytes = 0;
  try {
    for (const file of listRawSnapshots(directory).slice(RAW_SNAPSHOT_LIMIT)) {
      try {
...
function collectorHealth(options = {}) {
  ...
  const rawBacklog = rawRetentionBacklog(rawDir);
  return {
